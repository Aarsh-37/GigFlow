...
import { Briefcase, FileText, CheckCircle, Clock, IndianRupee, ArrowRight, TrendingUp, Wallet, Star, Loader2 } from 'lucide-react';
...
const Dashboard = ({ role: forcedRole, section }) => {
    ...
    if (isError) return (
        <div className="text-center py-20">
            <p className="text-red-500 font-bold text-xl">Failed to load dashboard stats.</p>
            <button onClick={() => queryClient.invalidateQueries(['dashboard-stats'])} className="mt-4 text-indigo-600 underline">Try again</button>
        </div>
    );
    ...
    return (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-10">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
                ...
                {!showToggle && (
                    <div className={`px-4 py-2 rounded-xl text-sm font-black ${role === 'hirer'
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                {activeTab === 'hirer' ? (
                    <>
                        ...
                        <StatCard title="Wallet Balance" value={`₹${(userInfo?.balance || 0).toLocaleString('en-IN')}`} icon={IndianRupee} color="bg-emerald-500" />
                    </>
                ) : (
                    <>
                                    ...
                                    <div key={active._id} className="flex items-center gap-4 p-4 rounded-2xl bg-gray-50 dark:bg-gray-700/50">
                                        ...
                                        <div className="flex-grow overflow-hidden">
                                            <p className="text-sm font-bold text-gray-900 dark:text-white truncate">
                                                {activeTab === 'hirer' ? active.internName : (active.gigId?.title || 'Unknown Project')}
                                            </p>
